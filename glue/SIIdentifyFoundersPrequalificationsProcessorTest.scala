...
import com.circle.data.jobs.SIIdentifyFoundersPrequalificationsProcessor
...
class SIIdentifyFoundersPrequalificationsProcessorTest
  ...
  val preqData = Seq(
    ...
    Row(2, "Alice", "Johnson", "+12345678902", "test1@email.com", "test2@email.com", timestamp1, timestamp1,
  ...
  test("Test identify founders with prequalifications") {
    ...
    val results = SIIdentifyFoundersPrequalificationsProcessor.getFoundersInfo(preqDF, profileDF, authDF)
    ...
    assert(timestamp1.equals(row1.getAs[Timestamp]("traits.created_at")))
  }
}
